...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function setModalContent(data) {
        ...
        if (data.title) {
            ...
            modalDate.textContent = date || "날짜 없음";
            
            if (data.id != null){
                modalBtn.innerHTML = `<button class="modal-btn edit-btn" onclick="window.location.href='/editDiary?id=${data.id}'">편집하기</button>`;
            } else {
                modalBtn.innerHTML = `<button class="modal-btn except-btn" disabled>편집할 수 없습니다.</button>`;
            }
        }
        // 일기가 없으면
        else {
